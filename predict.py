...
def load_checkpoint(filepath):
    ...
    model.classifier = nn.Sequential(
        nn.Linear(1280, checkpoint["hidden_units1"]),
        nn.ReLU(),
        nn.Dropout(0.2),
        nn.Linear(checkpoint["hidden_units1"], checkpoint["hidden_units2"]),
        nn.ReLU(),
        nn.Dropout(p=0.2),
        nn.Linear(checkpoint["hidden_units2"], checkpoint["hidden_units3"]),
        nn.ReLU(),
        nn.Dropout(p=0.2),
        nn.Linear(checkpoint["hidden_units3"], checkpoint["hidden_units4"]),
        nn.ReLU(),
        nn.Dropout(p = 0.2),
        nn.Linear(checkpoint["hidden_units4"], checkpoint['output_size']),
        nn.LogSoftmax(dim=1)
    )
